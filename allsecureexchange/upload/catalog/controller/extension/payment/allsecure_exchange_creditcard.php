...
include_once(DIR_SYSTEM . 'library/allsecure-exchange/autoload.php');

use AllsecureExchange\Client\Callback\Result;
...
final class ControllerExtensionPaymentAllsecureExchangeCreditCard extends Controller
{
    ...
    private $prefix = 'payment_allsecure_exchange_';
    ...
    {
        $data['type'] = $this->type;
        $data['action'] = $this->url->link('extension/payment/allsecure_exchange_' . $this->type . '/confirm', '', true);
        ...
        return $this->load->view('extension/payment/allsecure_exchange', $data);
    }
    ...
    public function confirm()
    {
        ...
        try {
            ...
            switch ($transactionRequest) {
                case AllsecureExchangePlugin::METHOD_PREAUTHORIZE:
                    $transaction = new Preauthorize();
                    break;
                case AllsecureExchangePlugin::METHOD_DEBIT:
                default:
     ...
     * @throws \AllsecureExchange\Client\Exception\InvalidValueException
     * @return \AllsecureExchange\Client\Client
     */
    private function client($cardType)
    {
        AllsecureExchange\Client\Client::setApiUrl(rtrim($this->getConfig('api_host'), '/') . '/');
        return new AllsecureExchange\Client\Client(
