...
include_once(DIR_SYSTEM . 'library/all-secure-exchange/autoload.php');
...
final class ControllerExtensionPaymentAllSecureExchangeCreditCard extends Controller
{
    use AllSecureExchangeGateway;
    ...
    private $prefix = 'payment_all_secure_exchange_';
    ...
    {
        $data['type'] = $this->type;
        $data['action'] = $this->url->link('extension/payment/all_secure_exchange_' . $this->type . '/confirm', '', true);

        $this->load->language('extension/payment/all_secure_exchange');
        ...
        return $this->load->view('extension/payment/all_secure_exchange', $data);
    }
    ...
    public function confirm()
    {
        ...
        try {
             ...
             * gateway client
             */
            AllSecureExchange\Client\Client::setApiUrl(rtrim($this->getConfig('api_host'), '/') . '/');
            ...
            $debit->setTransactionId($this->session->data['order_id'])
                ...
                ->setSuccessUrl(str_replace('&amp;', '&', $this->url->link('extension/payment/all_secure_exchange_' . $this->type . '/response', ['orderId' => $orderId, 'success' => 1])))
                ->setCancelUrl(str_replace('&amp;', '&', $this->url->link('extension/payment/all_secure_exchange_' . $this->type . '/response', ['orderId' => $orderId, 'cancelled' => 1])))
                ->setErrorUrl(str_replace('&amp;', '&', $this->url->link('extension/payment/all_secure_exchange_' . $this->type . '/response', ['orderId' => $orderId, 'failed' => 1])))
                ->setCallbackUrl(str_replace('&amp;', '&', $this->url->link('extension/payment/all_secure_exchange_' . $this->type . '/callback', 'orderId=' . $orderId, '&cardType=' . $cardType)));
